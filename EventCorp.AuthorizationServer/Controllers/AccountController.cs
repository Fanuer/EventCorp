...
namespace EventCorp.AuthorizationServer.Controllers
{
    ...
    public class AccountController : BaseApiController
    {
        ...
        /// </summary>
        /// <returns></returns>
        [Route("users/statistics", Name = "GetUserStatistics")]
        [SwaggerResponse(HttpStatusCode.Unauthorized, "You are not allowed to receive this resource")]
        [SwaggerResponse(HttpStatusCode.OK, Type = typeof(UserStatisticsModel))]
        [Authorize(Roles = "Admin")]
        [HttpGet]
        public async Task<IHttpActionResult> GetStatistics()
        {
            var adminRole = await this.AppRoleManager.Roles.FirstAsync(x => x.Name.ToLower().Equals("admin"));
            var result = new UserStatisticsModel
            {
                Url = (new UrlHelper(this.Request)).Link("GetUserStatistics", null),
                RegisteredUsers = await this.AppUserManager.Users.LongCountAsync(),
                RegisteredAdmins = await this.AppUserManager.Users.Include(x => x.Roles).Where(x => x.Roles.Any(y => y.RoleId == adminRole.Id)).LongCountAsync(),
                LockedOutUsers = await this.AppUserManager.Users.CountAsync(x => x.LockoutEnabled)
            };
            return Ok(result);
        }

        /// <summary>
        /// Get the EmailAddresses of AdminUsers
        /// </summary>
        /// <returns></returns>
        ...
        [HttpGet]
        [Route("admins/emails")]
        public async Task<IHttpActionResult> GetAdminEmailAddresses()
        {
            var adminRole = await this.AppRoleManager.Roles.FirstAsync(x => x.Name.ToLower().Equals("admin"));
            var emailAddresses =  await this.AppUserManager.Users
                                    .Include(x => x.Roles)
                                    .Where(x => x.Roles.Any(y => y.RoleId == adminRole.Id))
                                    .Select(x => x.Email)
                                    .ToListAsync();
